...
## 과제 요건에 대한 고민 및 접근/해결 방식
 - 보험 계약을 위한 상품 객체에 대한 고민을 먼저 하여 Porduct 1-N Coverage 구조의 간단한 상품 구조로 상품 객체를 표현 하였습니다.
 - 상품, 담보등은 보통 Code로 Mapping 을 시키지만, 직관성을 위해서 상품명, 담보명 을 Key 값으로 지정 하였습니다.
 - 보험계약객체 구조는 계약 1 - N 가입상품 1 - N 가입담보 의 관계로 표현 하였습니다.( 가입 상품의 경우 계약과 1 : 1구조로 표현해도 무관했지만, 추후 가입상품이 추가 될 수 있다는 가정하에 1:N 구조로 표현하였습니다. )
 - 보험기간 계산의 경우 시간은 모두 00:00분 이라고 가정 하였고, 1일부터 1개월로 가정 하였습니다. (1개월 이상 되는 시점 부터 2개월로 가정(1개월 1일 == 2개월 )
 - 계약번호는 @GeneratedValue(strategy = GenerationType.AUTO)옵션을 사용하여 생성 될 경우 삭제 후 다시 저장 시에 변경 가능성이 있어 채번테이블을 사용하여 직접 생성하였습니다. 
 - 계약 변경의 경우 배서 History 를 남겨야 하나, 해당 요건까지 세세히 진행하기에는 일정 상 불가능 하여 단순 업데이트로 처리 하였습니다.
 - 일정 상 상세한 Exception 은 추가하지 않고 null 로 리턴하였습니다.
